...
export default function CodeEditPage() {
  ...
  const copyToClipboard = (text: string, successMessage: string) => {
    ...
    navigator.clipboard?.writeText(text).then(() => {
      addToast({ title: "Copied", description: successMessage, color: "success" });
    }).catch(() => {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await cohortStorage.delete(codeId);
  ...
  const handleBack = () => router.push(isNew ? "/codes" : `/codes/${codeId}`);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className={pageTitle()}>{isNew ? "Create Cohort" : "Edit Cohort"}</h1>
      </div>
      ...
      <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                          accessMode === mode ? "border-primary bg-primary" : "border-default-300"
                        }`}
                      >
                        {accessMode === mode && <Check className="w-2.5 h-2.5 text-white" />}
                      </div>
                      <span className="text-sm">{ACCESS_MODE_LABELS[mode]}</span>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4 pt-4 border-t">
                ...
                <div className="flex items-center gap-4">
                  <label className="cursor-pointer">
                    <Button as="span" variant="bordered" startContent={<Upload className="w-4 h-4" />}>
                      Upload CSV
                    </Button>
                    <input type="file" accept=".csv" onChange={handleCsvUpload} className="hidden" />
                  </label>
                  <span className="text-sm text-default-500">
                    {students.length} learner{students.length !== 1 ? "s" : ""} added
                  </span>
                </div>
              </div>
            )}
        ...
        {/* Access Info for existing codes */}
        {!isNew && existing && (
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
            ...
            <CardBody className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-default-100 rounded-lg">
                ...
                <Button
                  ...
                  onPress={() => copyToClipboard(existing.accessCode, "Access code copied")}
                >
                  Copy
                </Button>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-default-100 rounded-lg">
                ...
                <Button
                  ...
                  size="sm"
                  startContent={<Copy className="w-4 h-4" />}
                  onPress={() => copyToClipboard(joinLink, "Join link copied")}
                >
                  Copy
                </Button>
              </div>
          ...
          <ModalBody className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                ...
                variant="bordered"
                startContent={<Copy className="w-4 h-4" />}
                onPress={() => copyToClipboard(createdAccessCode, "Access code copied")}
              >
                Copy Code
              </Button>
              <Button
                ...
                onPress={() => copyToClipboard(createdJoinLink, "Join link copied")}
              >
                Copy Link
              </Button>
            </div>
